...
# Oprava kategorií v DynamoDB - POUZE UŽIVATELSKÉ OTÁZKY (source: "user")
...
echo "   6→67→7, 7→78→8, 8→86→6 (pouze source=\"user\")"
echo ""
...
# Subject 6 -> 67 (pouze user)
if [ $COUNT_6 -gt 0 ]; then
    echo "   Subject 6 (user) -> 67 ($COUNT_6 položek)..."
    
    # Získání všech uživatelských položek s subjectId = 6
    ITEMS_6=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"6"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
# Subject 7 -> 78 (pouze user)
if [ $COUNT_7 -gt 0 ]; then
    echo "   Subject 7 (user) -> 78 ($COUNT_7 položek)..."
    
    ITEMS_7=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"7"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
# Subject 8 -> 86 (pouze user)
if [ $COUNT_8 -gt 0 ]; then
    echo "   Subject 8 (user) -> 86 ($COUNT_8 položek)..."
    
    ITEMS_8=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"8"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
COUNT_67=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"67"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
COUNT_78=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"78"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
COUNT_86=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"86"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
...
if [ $COUNT_67 -gt 0 ]; then
    ...
    ITEMS_67=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"67"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
if [ $COUNT_78 -gt 0 ]; then
    ...
    ITEMS_78=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"78"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
if [ $COUNT_86 -gt 0 ]; then
    ...
    ITEMS_86=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"86"},":src":{"S":"user"}}' --projection-expression "questionId" --region $REGION --query "Items[*].questionId.S" --output text)
...
FINAL_COUNT_6_USER=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"6"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
FINAL_COUNT_7_USER=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"7"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
FINAL_COUNT_8_USER=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"8"},":src":{"S":"user"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")

# Kontrola AI otázek (měly zůstat nezměněné)
FINAL_COUNT_6_AI=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"6"},":src":{"S":"ai"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
FINAL_COUNT_7_AI=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"7"},":src":{"S":"ai"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
FINAL_COUNT_8_AI=$(aws dynamodb scan --table-name $TABLE_NAME --filter-expression "subjectId = :sid AND source = :src" --expression-attribute-values '{":sid":{"N":"8"},":src":{"S":"ai"}}' --region $REGION --query "Count" --output text 2>/dev/null || echo "0")
